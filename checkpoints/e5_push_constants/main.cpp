...
void compute_shader() {
  ...
  shader_imageData[linearIndex] =  summedPixelColor / shader_push.num_samples;
}
...
int main(int argc, const char** argv)
{
  ...
  VkDeviceSize bufferSizeBytes = 
    uint64_t(pushConstants.resolution.x) * 
  ...
  vkCmdDispatch(cmdBuffer, (pushConstants.resolution.x + workgroup_width - 1) / workgroup_width,
  ...
  stbi_write_hdr("out.hdr", pushConstants.resolution.x, pushConstants.resolution.y, 
