...
void compute_shader() {
  const ivec2 resolution = shader_push.resolution;
  ...
  {
    averagePixelColor = 
      (shader_push.sample_batch * shader_imageData[linearIndex] + averagePixelColor) / 
      (shader_push.sample_batch + 1);
  }

  shader_imageData[linearIndex] =  averagePixelColor;
}
...
int main(int argc, const char** argv)
{
  ...
  VkDeviceSize bufferSizeBytes = uint64_t(pushConstants.resolution.x) * 
    uint64_t(pushConstants.resolution.y) * 3 * sizeof(float);
  ...
  for(uint32_t sampleBatch = 0; sampleBatch < NUM_SAMPLE_BATCHES; sampleBatch++)
  {
    ...
    vkCmdDispatch(cmdBuffer, 
      (pushConstants.resolution.x + workgroup_width - 1) / workgroup_width,
      (pushConstants.resolution.y + workgroup_height - 1) / workgroup_height, 1);
  ...
  stbi_write_hdr("out.hdr", pushConstants.resolution.x, 
    pushConstants.resolution.y, 3, reinterpret_cast<float*>(data));
